...
const SupersessionModeSchema = z.enum([
  SUPERSESSION_MODE.RETAIN,
  SUPERSESSION_MODE.AMEND,
  SUPERSESSION_MODE.REPLACE,
])
const SecondaryModeSchema = z
  .object({
    domain: z.literal(SUPERSESSION_DOMAIN.DEPENDENCY),
    mode: SupersessionModeSchema,
    tasks: z.array(NonEmptyStringSchema).nonempty(),
  })
  .strict()
  .or(
    z
      .object({
        domain: z.literal(SUPERSESSION_DOMAIN.HISTORICAL_EVIDENCE),
        mode: SupersessionModeSchema,
      })
      .strict(),
  )
  ...
  .object({
      ...
      .array(
        z
          .object({
            ...
            mode: SupersessionModeSchema,
            modeDomain: z
              .enum([SUPERSESSION_DOMAIN.SCOPE, SUPERSESSION_DOMAIN.DEPENDENCY, SUPERSESSION_DOMAIN.HISTORICAL_EVIDENCE])
              .default(SUPERSESSION_DOMAIN.SCOPE),
            secondaryModes: z.array(SecondaryModeSchema).default([]),
    ...
    reviewPolicy: z
      .object({
        ...
        primaryCodeOwner: z.literal(REVIEW_ACTOR.PRIMARY_CODE_OWNER),
        secondaryPrAuthor: z.string().optional(),
        requiredIndependentReview: z.literal(true),
        requiredCodeOwnerApproval: z.boolean(),
    ...
    guardrails: z
      .object({
        ...
        legacyQuiescenceMode: z.literal(LEGACY_QUIESCENCE_MODE.MAINTENANCE_NO_ORIGIN),
...
const DescriptorFixtureSchema = z
  .object({
    schemaVersion: z.literal(1),
    kind: z.literal(OVERLAY_FIXTURE_KIND.DESCRIPTOR),
    mutation: z.enum([
...
const LineageFixtureSchema = z
  .object({
    schemaVersion: z.literal(1),
    kind: z.literal(OVERLAY_FIXTURE_KIND.LINEAGE),
    mutation: z.enum([
      OVERLAY_LINEAGE_FIXTURE.DIRTY_UNSTAGED,
      OVERLAY_LINEAGE_FIXTURE.DIRTY_INDEX,
      OVERLAY_LINEAGE_FIXTURE.INTERMEDIATE_HEAD,
      OVERLAY_LINEAGE_FIXTURE.MERGE_HEAD,
    ]),
  })
  .strict()

export const OverlayFixtureSchema = z.discriminatedUnion("kind", [DescriptorFixtureSchema, LineageFixtureSchema])

export type DescriptorFixture = Readonly<z.infer<typeof DescriptorFixtureSchema>>
export type LineageFixture = Readonly<z.infer<typeof LineageFixtureSchema>>
