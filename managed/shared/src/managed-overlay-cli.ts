...
type CliOptions = Readonly<{
  ...
  readonly skipCorpus: boolean
  readonly json: boolean
}>

function usage(): never {
  throw new Error(
    `usage: npm run verify:managed-overlay -- ${ARGUMENT.PARENT} <parent-plan> ${ARGUMENT.OVERLAY} <overlay-plan> [${ARGUMENT.DESCRIPTOR} <path>] [${ARGUMENT.FIXTURE} <path>] [${ARGUMENT.SKIP_CORPUS}] [${ARGUMENT.JSON}]`,
  )
}
...
function parseArgs(argv: readonly string[]): CliOptions {
  ...
  return { parentPlanPath, overlayPlanPath, descriptorPath, fixturePath, skipCorpus, json }
}

function parseFixtureDescriptor(
  descriptor: ManagedOverlayDescriptor,
  fixturePath: string,
): Promise<ManagedOverlayDescriptor> {
  return readFile(fixturePath, "utf8").then((fixtureText) => {
    const fixture = parseOverlayFixture(JSON.parse(fixtureText) as unknown)
    return applyOverlayFixture(descriptor, fixture)
  })
}
...
async function verifyFixtureOnly(repositoryRoot: string, descriptorPath: string, fixturePath: string): Promise<void> {
  ...
  const descriptor = parseManagedOverlayDescriptor(JSON.parse(descriptorText) as unknown)
  const fixtureDescriptor = await parseFixtureDescriptor(descriptor, fixturePath)
  verifyManagedOverlayDescriptor(fixtureDescriptor)
  // A fixture is intentionally expected to fail; reaching this line is a failed negative oracle.
  throw new Error(`fixture unexpectedly passed: ${path.relative(repositoryRoot, fixturePath)}`)
}
...
async function main(): Promise<void> {
  ...
  const result = await verifyManagedOverlay({
    repositoryRoot,
    parentPlanPath: options.parentPlanPath as string,
    overlayPlanPath: options.overlayPlanPath as string,
    descriptorPath: options.descriptorPath,
    checkCorpus: !options.skipCorpus,
  })
  const output = {
    status: "VERIFIED",
