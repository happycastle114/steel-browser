...
receipt_path="${MANAGED_MANAGER_BUILD_RECEIPT:?MANAGED_MANAGER_BUILD_RECEIPT is required}"
...
if [[ ! "${candidate_repository}" =~ ^[a-z0-9][a-z0-9._/-]*$ ]]; then exit 64; fi
if [[ ! "${build_run_id}" =~ ^[a-z0-9][a-z0-9.-]{0,62}$ ]]; then exit 64; fi
case "${platform}" in
...
git archive --format=tar "${source_revision}" | docker buildx build \
  --file managed/manager/image/Dockerfile \
...
build_once() {
  local attempt="$1"
  local metadata="${build_root}/manager-${attempt}.metadata.json"
  ...
  local index_manifest="${build_root}/manager-${attempt}.index.json"
  ...
  local attestation_manifest="${build_root}/manager-${attempt}.attestations.json"
  ...
  local attestation_predicates="${build_root}/manager-${attempt}.predicates.ndjson"
  while read -r attestation_digest; do
    local attestation="${build_root}/manager-${attempt}-${attestation_digest#sha256:}.json"
  ...
  local platform_manifest="${build_root}/manager-${attempt}.platform.json"
